...
class CustomFileResource(DAVNonCollection):
    ...
    """
    def __init__(self, root: Directory, abspath: list[bytes], *, path: str, environ: dict):
    ...
    @property
    def disk(self) -> Disk: return self.root.disk
    @property
    def config(self) -> Config: return self.disk.config
    
    def assert_get_childs_inode(self, *names: bytes) -> Directory._InodeResult:
        result = self.root.get_childs_inode(*names)
        if result is None: 
            raise RuntimeError(f"{self.abspath=} is not found, TREE: {self.root.listtree()=}")
        return result
    ...
    def get_content_type(self) -> str: return util.guess_mime_type(self.path)
    ...
    def is_link(self) -> bool: return False # pyright: ignore[reportIncompatibleMethodOverride]
    def support_etag(self) -> bool: return True # pyright: ignore[reportIncompatibleMethodOverride]
    ...
    def delete(self) -> None:
        ...
        if not self._move_deleted:
            *parent_path, name = self.abspath # auto prevent root delete
            ...
            util._logger.debug(f"delete(): {self.path} already gone, skipping; {self._move_deleted=}")
    ...
    def set_last_modified(self, dest_path: str, time_stamp: str, *, dry_run: bool) -> bool:
        ...
        if secs is None: raise ValueError(f"Unable to Parse {time_stamp=}")
        if not dry_run: 
            result = self.assert_get_childs_inode(*self.abspath)
