...
class EncryptedBytesIOWrapper(BytesIO):
    ...
    def closed(self) -> bool:
        return self._closed or self.file.closed
    ...
    def _read_raw(self, size: int = -1) -> bytes:
        return self.file.read(size)

    def _write_raw(self, buffer: ByteString) -> int:
        return self.file.write(buffer)

    def read(self, size: int = -1) -> ByteString:  # type: ignore[override]
    ...
    def write(self, buffer: ByteString) -> int:  # type: ignore[override]
        ...
        if self._gap_size > 0:
            ...
            filler: ByteString = self.encryptor.encrypt(NULL_BYTES * self._gap_size)
        ...
        data: ByteString = self.encryptor.encrypt(buffer)
...
class InFileChaCha20EncryptedDisk(InFileDisk):
    def __init__(self, filepath: str | BinaryIO, password: bytes) -> None:
        ...
        try:
            disk_type = InFileDiskType(
                int.from_bytes(self.file.read(1), byteorder="big", signed=False)
            )
        except ValueError:
            raise TypeError(
        ...
        self.file = EncryptedBytesIOWrapper(
            encryptor, decryptor, self.file, auto_close=True
        )
        self.config = load_config_from_file(self.file)

        len_config_bytes: int = self.file.tell()

        self.inodes_bitmap = BitmapFile(
            self.config.num_inodes, self.file, pos=len_config_bytes
        )
        self.blocks_bitmap = BitmapFile(
        ...
        num_super_blocks: int = ceil_division(
            header_size_required, self.config.block_size
        )
        if num_super_blocks == 0:
            raise RuntimeError(
                f"Something went wrong, {num_super_blocks=} should not be zero, matbe disk is corrupted."
            )
        for i in range(num_super_blocks):
            if not self.blocks_bitmap.get(i):
                raise RuntimeError(
                    f"Something went wrong, {self.blocks_bitmap.get(i)=}[{i=}] should be set, matbe disk is corrupted."
                )

        if not self.inodes_bitmap.get(0):
            raise RuntimeError(
                f"Something went wrong, {self.inodes_bitmap.get(0)=} should be set, matbe disk is corrupted."
            )
        ...
        root_inode = Inode.from_bytes(data=self.inodes[0], config=self.config)
        self.root = Directory(disk=self, inode=root_inode, inode_ptr=0)

    @classmethod
    def new_disk(
        cls, filepath: str | BinaryIO, config: Config, password: bytes | None = None
    ) -> Self:
        if password is None:
            raise ValueError(f"{password=} must not be None.")
        ...
        auth_tag: bytes = HkdfHmac.make(
            password=password, nonce=nonce, info=self.__class__.__name__.encode("utf-8")
        )
        ...
        if isinstance(filepath, str):
            ...
            self.file = EncryptedBytesIOWrapper(
                encryptor, decryptor, file=open(filepath, "wb+"), auto_close=True
            )
        else:
            self.file = EncryptedBytesIOWrapper(
                encryptor, decryptor, file=filepath, auto_close=True
            )
        self._closed = False
        self.file._write_raw(
            InFileDiskType.CHA_CHA_20_ENCRYPTED.value.to_bytes(
        ...
        self.inodes_bitmap = BitmapFile(
            config.num_inodes, self.file, pos=len_config_bytes
        )
        self.blocks_bitmap = BitmapFile(
            config.num_blocks,
            self.file,
        ...
        header_size_required: int = (
            header_prefix_size_required + config.inode_size * config.num_inodes
        )
        self._reserved_space = header_size_required
        ...
        if num_super_blocks == 0:
            raise RuntimeError(
                f"Something went wrong, {num_super_blocks=} should not be zero."
            )
        for i in range(num_super_blocks):
