...
class HkdfHmac:
    ...
    def make(cls, password: bytes, nonce: bytes, info: bytes) -> bytes:
        ...
        return auth_tag
    @classmethod
    def verify(cls, password: bytes, nonce: bytes, info: bytes, stored_tag: bytes) -> bool:
        if len(stored_tag) != cls.HMAC_SIZE:
            raise ValueError(
                f"{len(stored_tag)=} must be of size: {cls.HMAC_SIZE}"
            )
        auth_key: bytes = HKDF(
            algorithm=hashes.SHA256(),
            length=cls.HMAC_SIZE,
...
class CipherChaCha20:    
    ...
    def seek(self, offset: int = 0, /):
        ...
        self.context.reset_nonce(
            nonce=(
                block_counter.to_bytes(4, "little") + 
                self._nonce
            )
        )
